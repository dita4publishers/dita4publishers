...
<xsl:stylesheet
  ...
  exclude-result-prefixes="df xs relpath kindleutil opf dc xd"
  version="2.0">
  <!-- =============================================================
  ...
  <xsl:template match="*[df:class(., 'map/map')]" mode="generate-content">
    ...
    <xsl:apply-templates select="df:getUniqueTopicrefs(.)" mode="generate-content"/>
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        ...
        <xsl:apply-templates select="$tempTopic" mode="#current">
          ...
          <xsl:with-param name="resultUri" select="kindleutil:getTopicResultUrl($topicsOutputPath, root($topic))"
           tunnel="yes"/>
        </xsl:apply-templates>
      </xsl:otherwise>
    </xsl:choose>    
