...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="/">
		...
		<xsl:for-each select="//ContentGroup/ContentFile">
			...
			<!-- set up where we find and save the content, because it depends on the param samedir -->
			...
			<xsl:variable name="wherecreate">
				<xsl:choose>
					<xsl:when test="$samedir!='no'">/Items/</xsl:when>
					<xsl:otherwise>/</xsl:otherwise>
				</xsl:choose>
			</xsl:variable>

			<xsl:variable name="items_path">
				<xsl:value-of select="concat($CourseId,$wherecreate)" />
			</xsl:variable>
									...
									<table class="categoryboxcontent">
										<tbody>
											...
											<tr>
												<td>
													<xsl:choose>
														<xsl:when test="contains($pageType,'index')">
